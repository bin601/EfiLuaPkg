...
#define IS_CONST(tok) (IS_LITERAL(tok, "const") || IS_LITERAL(tok, "__const") || IS_LITERAL(tok, "__const__") || IS_LITERAL(tok, "CONST"))
...
start:
    ...
    if (*s == '.' || *s == '-' || ('0' <= *s && *s <= '9')) {
        ...
        if (IS_LITERAL (*tok, "IN") || IS_LITERAL (*tok, "OUT") || IS_LITERAL (*tok, "OPTIONAL")) {
          goto start;
        }
...
#error
        ...
        *pbitfield_type = -1;
		...
		if (mt->pointers) {
			...
			ct->base_size = ALIGN_UP(ct->base_size, ALIGNOF(aptr));
		} else {
			ct->base_size = ALIGN_UP(ct->base_size, mt->align_mask);
		}
        mt->offset = ct->base_size;
    ...
    if (mt->pointers) {
      struct {char ch; uintptr_t v;} aptr;
      int align = ALIGNOF(aptr);
      if (align > (int) ct->align_mask) {
          ct->align_mask = align;
      }
    } else {
...
static int parse_attribute(lua_State* L, struct parser* P, struct token* tok, struct ctype* ct, struct parser* asmname)
{
    if (tok->type != TOK_TOKEN) {
        ...
        *asmname = *P;
        ...
        for (;;) {
            ...
            } else if (IS_LITERAL(*tok, "cdecl") || IS_LITERAL(*tok, "__cdecl__") || IS_LITERAL(*tok, "EFIAPI")) {
                //
                // need check EFIAPI
                //
    ...
    } else if (IS_LITERAL(*tok, "__cdecl") || IS_LITERAL(*tok, "EFIAPI")) {
        ct->calling_convention = C_CALL;
        return 1;
...
static int parse_root(lua_State* L, struct parser* P)
{
    ...
    while (next_token(L, P, &tok)) {
        ...
        if (tok.type == TOK_SEMICOLON) {
            ...
            } else if (tok.type == TOK_CLOSE_PAREN) { // #pragma pack ()
